...
import com.salesmore.yak.integration.fyndiq.api.EndpointTokenProvider;
...
/**
 * A Request Delegate which aids in building the request that is compatible with the Rest API.
 * The request is used to encoding as well as keeping reference to
 * the return type
 * 
...
public class HttpRequest<R> {
	...
	public static final class RequestBuilder<R> {
		...
		public HttpRequest<R> build() {
            if (provider != null)
            {
                request.endpoint = provider.getEndpoint();
                if (provider.getTokenId() != null && provider.getAuthHeaderName() != null)
                    request.getHeaders().put(provider.getAuthHeaderName(), provider.getTokenId());
            }
			return request;
		}
	}
